...
def  minimalEncoding():
    ...
    for i in range(1, 10):  # Iterate over rows
        for k in range(1, 10):  # Iterate over numbers
            clause = [getVariables(i, j, k) for j in range(1, 10)]  # Include variables for each cell in the row
            cnf.append(clause)

    # Every number appears at least once in each column
    for j in range(1, 10):  # Iterate over columns
        for k in range(1, 10):  # Iterate over numbers
            clause = [getVariables(i, j, k) for i in range(1, 10)]  # Include variables for each cell in the column
            cnf.append(clause)


    # Every number appears at least once in each subgrid
    for c in range(1, 10):  # Iterate over numbers
        for a in range(3):  # Iterate over subgrid rows
            for b in range(3):  # Iterate over subgrid columns
                clause = []
                for d in range(1, 4):  # Iterate over cells within subgrid
                    for e in range(1, 4):
                        clause.append(getVariables(3*a + d, 3*b + e, c))
                cnf.append(clause)
